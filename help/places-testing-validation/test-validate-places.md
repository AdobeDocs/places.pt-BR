...
>[!IMPORTANT]
>
>Este plano pressupõe que os POIs foram criados no [Interface do usuário do Places Service](https://places.adobe.com) e as versões mais recentes da extensão Places estão instaladas e configuradas corretamente. Se estiver fazendo o monitoramento de região ativo, ele também pressupõe que uma solução de monitoramento de região esteja implementada. Para obter mais informações, consulte [Extensão do Places](/help/places-ext-aep-sdks/places-extension/places-extension.md), [Documentação do CoreLocation](https://developer.apple.com/documentation/corelocation/monitoring_the_user_s_proximity_to_geographic_regions) para o iOS ou [Documentação de localização do Android](https://developer.android.com/training/location/geofencing).

| Etapa | Descrição | Resultado esperado |
|--- |--- |--- |
| 1 | Confirme se as chaves de manifesto apropriadas foram inseridas para que o Android conceda acesso à localização de rastreamento. | Confirmado |
...
| 4 | Confirme se as versões mais recentes das extensões Mobile Core e Places foram fornecidas com o aplicativo por meio da Gradle ou da CocoaPods. | Confirmado - para obter mais informações sobre atualizações recentes, consulte a [notas de versão.](/help/release-notes.md) |
...
| 7 | Sem fazer mais nada, você deve ser capaz de iniciar o aplicativo a partir do Android Studio ou XCode e ver o alerta apropriado para solicitar acesso ao local de rastreamento. Clique em *Sempre permitir* permissão.<br><br>Recomendamos que você use um dispositivo real que esteja conectado ao seu computador em vez de usar o simulador de dispositivo. | O prompt da solicitação de localização deve ser exibido no aplicativo carregado pelo IDE |
...
| 10a | No Launch, verifique se você adicionou elementos de dados para a extensão do Places, incluindo o seguinte: <br>Nome do POI atual<br>Último POI atual<br>POI longo atual<br>Sobrenome inserido<br>Última entrada<br>Última Entrada longa<br>Nome da última saída<br>Última saída<br>Última saída longa<br>Carimbo de data e hora |  |
...
|  | **PONTO DE RESUMO RÁPIDO**<br> Todos esses testes podem ser conduzidos localmente sem precisar ir para um local específico do POI. O teste de validação ajuda a garantir que seu aplicativo esteja configurado corretamente e tenha recebido as permissões corretas para o local. <br><br>Essa validação também garante que os POIs definidos estejam funcionando corretamente com a implementação de monitoramento de sua região.  Após essa etapa, começaremos a testar as mensagens no Campaign para ver se as mensagens apropriadas são exibidas com base nas entradas e saídas do POI. |  |
|  | **Teste de mensagens no aplicativo do Adobe Campaign Standard com o serviço Places.** |  |
...
| 12b | Selecionar **[!UICONTROL Metadados personalizados do Places]** como filtro adicional - use o tipo de POI = Last Entered POI (Último POI inserido).<br>Usamos **[!UICONTROL Última Entrada]** como o tipo de POI porque, na maioria dos casos, **[!UICONTROL Última Entrada]** será o mesmo que **[!UICONTROL POI atual]**. <br><br>**[!UICONTROL POI atual ]**O só deve ser usado em instâncias onde há cercas geográficas de POI sobrepostas. Nesse caso, esses POIs precisam ser CLASSIFICADOS e depois a variável**[!UICONTROL  POI atual ]**exibirá o POI mais bem classificado dentre as 2 ou 3 cercas geográficas nas quais um usuário pode estar no momento. |  |
...
| 12e | Em Sempre/Uma vez ou Até o click-through, selecione *SEMPRE* para que você possa testar em vários locais. | Uma mensagem no aplicativo é exibida SEMPRE quando você simula uma alteração de local que atende aos critérios de metadados apropriados. |
...
| 16a | Em acionadores, selecione **[!UICONTROL Tipo de evento de locais]** - **[!UICONTROL Entrada como acionador]**. |  |
| 16b | Selecione os metadados personalizados do Places como um filtro adicional - use **[!UICONTROL Tipo de POI]** = **[!UICONTROL Último POI de entrada]**. |  |
...
| 16e | Para Always/Once ou Until click-through, **[!UICONTROL SEMPRE]**. |  |
